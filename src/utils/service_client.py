...
from src.middlewares.correlation_id import get_correlation_id, create_headers_with_correlation_id
...
class ServiceClient:
    ...
    async def get(self, endpoint: str, headers: Optional[Dict[str, str]] = None, **kwargs) -> httpx.Response:
...
user_service_client = ServiceClient(os.getenv('USER_SERVICE_URL', 'http://localhost:5000'))
auth_service_client = ServiceClient(os.getenv('AUTH_SERVICE_URL', 'http://localhost:4000'))
inventory_service_client = ServiceClient(os.getenv('INVENTORY_SERVICE_URL', 'http://localhost:3000'))
order_service_client = ServiceClient(os.getenv('ORDER_SERVICE_URL', 'http://localhost:7000'))
payment_service_client = ServiceClient(os.getenv('PAYMENT_SERVICE_URL', 'http://localhost:8080'))
admin_service_client = ServiceClient(os.getenv('ADMIN_SERVICE_URL', 'http://localhost:6000'))


async def log_with_correlation_id(level: str, message: str, extra: Optional[Dict[str, Any]] = None):
