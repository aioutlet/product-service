...
class InventoryServiceClient:
    ...
    async def get_inventory_by_product_id(self, product_id: str) -> Optional[InventoryItem]:
        ...
        """
        try:
            response = await self.client.get(f"{self.base_url}/inventory/product/{product_id}")
            if response.status_code == 404:
                return None
            response.raise_for_status()
            data = response.json()
    ...
    async def is_sku_available(self, sku: str, quantity: int = 1) -> bool:
        ...
        """
        try:
            response = await self.check_stock([StockCheckItem(sku=sku, quantity=quantity)])
            return response.available and len(response.items) > 0 and response.items[0].available
