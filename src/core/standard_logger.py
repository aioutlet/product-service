    ...
    def format(self, record):
        ...
        for key, value in record.__dict__.items():
            if key not in ['name', 'msg', 'args', 'levelname', 'levelno', 'pathname', 'filename',
                          ...
                          'exc_text', 'stack_info', 'message', 'correlationId', 'userId', 'operation', 'duration']:
                if value is not None:
                    meta_fields.append(f"{key}={json.dumps(value) if isinstance(value, (dict, list)) else value}")
        
        meta_str = f" | {', '.join(meta_fields)}" if meta_fields else ""
        
        base_msg = f"[{timestamp}] [{record.levelname}] {SERVICE_NAME} {corr_id_str}: {record.getMessage()}{meta_str}"
    ...
    def format(self, record):
        ...
        for key, value in record.__dict__.items():
            if key not in ['name', 'msg', 'args', 'levelname', 'levelno', 'pathname', 'filename',
                          'module', 'lineno', 'funcName', 'created', 'msecs', 'relativeCreated',
                          'thread', 'threadName', 'processName', 'process', 'getMessage', 'exc_info',
                          'exc_text', 'stack_info', 'message'] and value is not None:
...
class StandardLogger:
    ...
    def __init__(self):
        ...
        # Log initialization
        self._log("info", "Logger initialized", metadata={
            "logLevel": LOG_LEVEL,
    ...
    def _log(self, level: str, message: str, request=None, metadata: Optional[Dict[str, Any]] = None):
    ...
    def info(self, message: str, request=None, metadata: Optional[Dict[str, Any]] = None):
        """Info level logging"""
        self._log("info", message, request, metadata)
    
    def debug(self, message: str, request=None, metadata: Optional[Dict[str, Any]] = None):
        """Debug level logging"""
        self._log("debug", message, request, metadata)
    
    def warning(self, message: str, request=None, metadata: Optional[Dict[str, Any]] = None):
        """Warning level logging"""
        self._log("warning", message, request, metadata)
    
    def error(self, message: str, request=None, metadata: Optional[Dict[str, Any]] = None):
        """Error level logging"""
        if metadata is None:
            metadata = {}
    ...
    def fatal(self, message: str, request=None, metadata: Optional[Dict[str, Any]] = None):
    ...
    def operation_start(self, operation: str, request=None, metadata: Optional[Dict[str, Any]] = None) -> float:
    ...
    def operation_complete(self, operation: str, start_time: float, request=None, metadata: Optional[Dict[str, Any]] = None) -> float:
    ...
    def operation_failed(self, operation: str, start_time: float, error, request=None, metadata: Optional[Dict[str, Any]] = None) -> float:
    ...
    def business(self, event: str, request=None, metadata: Optional[Dict[str, Any]] = None):
        """Log business events"""
        if metadata is None:
            metadata = {}
        metadata.update({
    ...
    def security(self, event: str, request=None, metadata: Optional[Dict[str, Any]] = None):
        """Log security events"""
        if metadata is None:
            metadata = {}
        metadata.update({
    ...
    def performance(self, operation: str, duration: int, request=None, metadata: Optional[Dict[str, Any]] = None):
